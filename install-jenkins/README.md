...
4. Wire up [`sample-java-app`](../sample-java-app) CI:

   | Step | Action |
   |------|--------|
   | Credential | **Manage Jenkins → Credentials** → Secret text, ID **`sonarqube-token`** (token from SonarQube) |
   | Job | **New Item** → Pipeline → **Pipeline script from SCM** |
